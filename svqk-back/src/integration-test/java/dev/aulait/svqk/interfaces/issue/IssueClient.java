...
public class IssueClient {
  ...
  public IssueDto save(IssueDto issue) {
    return given().body(issue).post(ISSUES_PATH).then().statusCode(200).extract().as(IssueDto.class);
  }

  public ConstraintViolationResponseDto createButValidationError(IssueDto issue) {
    return given().body(issue).post(ISSUES_PATH).then().statusCode(400).extract()
